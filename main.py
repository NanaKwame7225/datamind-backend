...
FastAPI Backend
Endpoints: GET /health  |  POST /api/v1/analysis/analyse
AI Engine: Claude (primary) → OpenAI GPT-4o Mini (fallback) → Statistical (offline)
"""
...
def _base_prompt(req: AnalyseRequest) -> str:
    nk = _numeric_keys(req.inline_data)
    anomaly_summary = _detect_anomalies(req.inline_data, nk)
    return f"""You are DataMind, an expert AI data auditor for the {req.industry} sector.
...
Query: {req.query}
Data sample (first 8 records): {json.dumps(req.inline_data[:8], default=str)}
"""
...
# ── OpenAI ─────────────────────────────────────────────────────────────────
async def _call_openai(req: AnalyseRequest) -> dict:
    ...
    return {
        ...
        "insights":         ai_data.get("insights",  []),
        "charts":           ai_data.get("charts",    []),
        "pipeline_steps":   _build_pipeline("GPT-4o Mini", elapsed_ms),
        "execution_ms":     round(elapsed_ms),
        "raw_data_preview": req.inline_data[:6],
    }
...
def _statistical_fallback(req: AnalyseRequest) -> dict:
    ...
    nar = (
        ...
        f"{iname} sector norms. Connect an AI API key (Anthropic or OpenAI) for a deeper analysis."
    )
...
async def analyse(req: AnalyseRequest):
    ...
    if OPENAI_API_KEY:
        try:
            ...
            log.warning("OpenAI failed: %s", e)

    # 3. Statistical fallback — always works
